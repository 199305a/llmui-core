...
def main():
    ...
    success, _ = execute_command(
        f"git clone {GITHUB_REPO} {temp_dir}", "Clonage du dépôt GitHub"
    )
    ...
    if os.path.exists(f"{temp_dir}/src"):
        execute_command(
            f"sudo cp -r {temp_dir}/src /opt/llmui-core/", "Copie répertoire src"
        )
    else:
    ...
    if os.path.exists(f"{temp_dir}/web"):
        execute_command(
            f"sudo cp -r {temp_dir}/web /opt/llmui-core/", "Copie répertoire web"
        )
    else:
    ...
    if os.path.exists(f"{temp_dir}/requirements.txt"):
        ...
        try:
            ...
            log(
                "📦 Installation des dépendances additionnelles du requirements.txt...",
                "INFO",
            )
            execute_command(
    ...
    execute_command("sudo mkdir -p /opt/llmui-core/logs", "Création répertoire logs")
    ...
    execute_command("sudo chown -R llmui:llmui /opt/llmui-core/src", "Permissions src")
    execute_command("sudo chown -R llmui:llmui /opt/llmui-core/web", "Permissions web")
    execute_command(
        "sudo chown -R llmui:llmui /opt/llmui-core/logs", "Permissions logs"
    )
    execute_command(
        "sudo chown -R llmui:llmui /opt/llmui-core/venv", "Permissions venv"
    )

    if os.path.exists("/opt/llmui-core/config.yaml"):
        execute_command(
            "sudo chown llmui:llmui /opt/llmui-core/config.yaml", "Permissions config"
        )
        execute_command("sudo chmod 600 /opt/llmui-core/config.yaml", "Chmod config")
